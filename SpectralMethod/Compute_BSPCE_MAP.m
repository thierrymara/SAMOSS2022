...
for r=2:size(Matdegre1,1)
    ...
    if ~isempty(Ind)
        ...
        Sigma2P=[Sigma2P;2.^(-(sum(Matdegre(end,:))+4*(length(Ind)-1)))];%1/((sum(Matdegre(end,Ind))+length(Ind)-1)*length(Ind)^q)];%
    else
        disp('Erreur quelque part : voir Matdegre')
    end
    Sigma2e=1;
    ...
    while max(abs(Sigma2e_prec-Sigma2e)./Sigma2e)>0.0001
        ...
        Cr1=((Matrice')*Matrice+Sigma2e*diag(1./Sigma2P))\((Matrice')*y_std);%inv((Matrice')*Matrice+Lambda*diag(1./Sigma2P))*(Matrice')*y_std;%
    ...
    while or(isinf(DetJ),DetJ==0)
        DetJ=det((Matrice')*Matrice/(Nsample^N*Sigma2e)+diag(1./(Nsample^N*Sigma2P)));%I divide by Nsample^N to avoid DetJ=Inf. This is corrected below in the KIC
        if isinf(DetJ)
            N=N+1;
        elseif DetJ==0
            N=N-1/20;
    ...
    KIC=Nsample*log(Sigma2e)+N*size(Matrice,2)*log(Nsample)+log(abs(DetJ))+sum(log(Sigma2P))+sum(Cr1.^2./Sigma2P);%Kashyap IC, as small as possible
    ...
    if (KIC<KIC_Prec)&(Cr1(end)^2>Cr1(1)^2/10)%%%%%%%%%%%%%%%%%%%%%%
        ...
        Cr1=((Matrice')*Matrice+Sigma2e*diag(1./Sigma2P))\((Matrice')*y_std);%inv((Matrice')*Matrice+Lambda*diag(1./Sigma2P))*(Matrice')*y_std;%Biased estimate
    end
end
...
SPCE(1).Cov=inv((Matrice')*Matrice/Sigma2e+diag(1./Sigma2P));
